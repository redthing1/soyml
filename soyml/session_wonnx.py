...
def session_wonnx_init(self):
    try:
        logger.debug(f"[session_wonnx] loading wonnx model: {self.wonnx_model_file}")
        ...
        logger.debug(f"[session_wonnx] wonnx input shapes: {self.input_shapes}")
        logger.debug(f"[session_wonnx] wonnx output shapes: {self.output_shapes}")
        del onnx_model
...
def session_wonnx_execute(self, inputs, output_names):
    logger.debug(f"[session_wonnx] execute: {inputs.keys()} -> {output_names}")
    try:
